...
function WeeklyPlanner() {
  ...
  async function fetchActivities() {
    try {
      const response = await fetch('http://localhost:8000/api/volunteer/activites_prives', {
        method: 'GET',
        credentials: 'include'
      });
      if (!response.ok) {
          ...
          <tbody>
            {hours.map((hour, hourIndex) => (
              <tr key={hourIndex}>
                ...
                {[0, 1, 2, 3, 4, 5, 6].map((dayIndex) => (
                  <td key={dayIndex}>
                    {/* Render activities for this hour and day */}
...
{selectedActivity && (
  <div className="popup">
    <div className="popup-content">
      ...
      <p>Lieu: {selectedActivity.id_lieu}</p>
